...
export default function TrackingPage() {
  ...
  return (
    <div>
      <h2 style={{ fontSize:20, fontWeight:700, color:"var(--color-ink-primary)", letterSpacing:"-0.3px", marginBottom:16 }}>Email Tracking</h2>

      {/* Daily usage bar */}
      <div style={s.usageCard}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
          <p style={{ fontSize:13, fontWeight:600, color:"var(--color-ink-primary)" }}>
            Emails sent today: <span style={{ color: dailySent >= dailyLimit ? "var(--color-error)" : "var(--color-accent-terracotta)" }}>{dailySent}</span>
            <span style={{ color:"var(--color-ink-muted)" }}> / {dailyLimit}</span>
          </p>
          <a href="/settings" style={{ fontSize:12, color:"var(--color-ink-mid)", textDecoration:"none", fontWeight:500 }}>Edit limit →</a>
        </div>
        <div style={s.usageTrack}>
          <div style={{ ...s.usageFill, width:`${Math.min(100, (dailySent / dailyLimit) * 100)}%`, background: dailySent >= dailyLimit ? "var(--color-error)" : "var(--color-accent-terracotta)" }} />
        </div>
        {queuedTomorrow > 0 && (
      ...
      <div style={{ ...s.statsRow, gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(4,1fr)", marginBottom:28 }}>
        {[
          { label:"Hotels Reached",   value: totalHotels,      color:"var(--color-ink-primary)" },
          { label:"Replies Received", value: totalReplied,     color:"var(--status-success-ink)" },
          { label:"Active Jobs",      value: totalActive,      color:"var(--color-accent-terracotta)" },
              ...
              <Link key={seq.id} href={`/sequences/tracking/${seq.id}`} style={{ textDecoration:"none" }}>
                <div style={s.seqCard}>
                  ...
                  <div style={s.seqStats}>
                    <div style={s.seqStat}>
                      <span style={{ ...s.seqStatDot, background:"var(--color-action-forest)" }} />
                      <span style={s.seqStatLabel}>{active} active</span>
                    </div>
                    <div style={s.seqStat}>
                      <span style={{ ...s.seqStatDot, background:"var(--status-success-ink)" }} />
                      <span style={s.seqStatLabel}>{replied} replied</span>
                    </div>
                    <div style={s.seqStat}>
                      <span style={{ ...s.seqStatDot, background:"var(--status-sent-ink)" }} />
                      <span style={s.seqStatLabel}>{completed} done</span>
                    </div>
                    {cancelled > 0 && (
                      <div style={s.seqStat}>
                        <span style={{ ...s.seqStatDot, background:"var(--color-border)" }} />
                  ...
                  {total > 0 && (
                    <div style={{ marginTop:14 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
                        <span style={{ fontSize:11, color:"var(--color-ink-muted)" }}>Reply rate</span>
                        <span style={{ fontSize:11, fontWeight:700, color:"var(--status-success-ink)" }}>{rate}%</span>
                      </div>
...
const s = {
  usageCard: { background:"var(--color-ground-card)", borderRadius:14, border:"1px solid var(--color-border)", padding:"16px 20px", marginBottom:20 },
  usageTrack: { height:8, background:"var(--color-ground-sand)", borderRadius:99, overflow:"hidden" },
  ...
  seqCard: { background:"var(--color-ground-card)", borderRadius:14, border:"1px solid var(--color-border)", padding:"18px 20px", cursor:"pointer", transition:"box-shadow 0.15s, transform 0.15s" },
  seqName: { fontSize:15, fontWeight:700, color:"var(--color-ink-primary)", marginBottom:3 },
  seqMeta: { fontSize:12, color:"var(--color-ink-muted)" },
  ...
  seqStatLabel: { fontSize:12, color:"var(--color-ink-mid)" },
  progressTrack: { height:5, background:"var(--color-ground-sand)", borderRadius:99, overflow:"hidden" },
  progressFill: { height:"100%", background:"var(--status-success-ink)", borderRadius:99, transition:"width 0.4s ease" },
