...
// One Gmail connection, shared app-wide. Previously every caller of
// useGmail() ran its own independent copy of this state — Settings,
// Messages, and the Flows launch modal each started at gmailToken=null and
// had to reconnect from scratch on mount. Navigating between them looked
// like getting logged out of Gmail, even though the stored connection never
// actually went away; the new page's private copy just hadn't caught up yet.
// Mounting this once at the layout level and reading it via context fixes
// that: the connection is fetched once and stays live across navigation.
export function GmailProvider({ children }) {
  ...
  // Restore the connection whenever a Supabase session becomes available.
  useEffect(() => {
    // Try immediately in case the session is already ready.
    refresh();

    // Also listen for sign-in events (covers the case where the hook mounts
    // before Supabase has finished restoring the session from storage).
  ...
  return (
    <GmailContext.Provider value={{ gmailToken, gmailEmail, gmailLoading, tokenExpired, connectGmail, disconnectGmail }}>
      {children}
